...
const createBlock = (type: CaseStudyBlock["type"]): CaseStudyBlock => {
  switch (type) {
    case "textblock":
      return { type: "textblock", content: "" };
    case "image":
      return { type: "image", src: "", alt: "" };
    case "video":
      return { type: "video", src: "", poster: "" };
    case "twoImages":
      return { type: "twoImages", images: Array(2).fill({ src: "", alt: "" }) };
    case "compareFourImages":
      return {
        type: "compareFourImages",
        images: Array(4).fill({ src: "", alt: "" }),
      };
    case "compareFiveImages":
      return {
        type: "compareFiveImages",
        images: Array(5).fill({ src: "", alt: "" }),
      };
    case "compareSevenImages":
      return {
        type: "compareSevenImages",
        images: Array(7).fill({ src: "", alt: "" }),
      };
    default:
      return { type: "textblock", content: "" };
  }
...
export default function CaseStudyEditor({
  ...
  function removeBlock(pIndex: number, bIndex: number) {
    setProjects((prev) =>
      prev.map((p, i) =>
        i === pIndex
          ? { ...p, content: p.content.filter((_, bi) => bi !== bIndex) }
          : p,
      ),
    );
  }
  ...
  function updateMultiImageBlock(
    pIndex: number,
    bIndex: number,
    imgIndex: number,
    field: "src" | "alt",
  ...
  return (
    <form
      ...
      className="space-y-8 p-6 bg-[#111] text-white rounded-xl border border-white/10 text-sm"
    >
      {/* Title Section */}
      <div className="border-b border-white/10 pb-6">
        <label className="block text-small font-bold uppercase text-white/50 mb-2">
          Section Title
        </label>
        <input
          value={title}
          ...
          >
            ...
            <div className="bg-white/5 p-4 flex justify-between items-center border-b border-white/10">
              ...
              <button
                ...
                className="text-small font-bold text-red-400 hover:text-red-300 border border-red-500/30 px-3 py-1 rounded bg-red-500/10"
              >
                Delete Project
              </button>
            </div>
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-small text-white/50">
                    Project Name
                  </label>
                  <input
                    value={proj.title}
                    onChange={(e) =>
                      updateProjectHeader(pIndex, "title", e.target.value)
                    }
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateProjectHeader(pIndex, "projectLink", e.target.value)
                    }
                    className="w-full bg-black/50 border border-white/10 rounded px-3 py-2 outline-none focus:border-purple-500"
                    placeholder="https://..."
                  />
                ...
                <div className="col-span-2 space-y-1">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateProjectHeader(pIndex, "src", e.target.value)
                    }
                    className="w-full bg-black/50 border border-white/10 rounded px-3 py-2 outline-none focus:border-purple-500"
                  />
                </div>
              ...
              <div className="mt-8 space-y-4 pl-4 border-l-2 border-purple-500/20">
                <div className="text-small font-bold uppercase text-white/40 tracking-widest">
                  Content Blocks
                </div>
                ...
                {proj.content.map((block, bIndex) => (
                  <div
                    key={bIndex}
                    className="relative bg-black/40 rounded border border-white/10 p-4 group"
                  >
                    ...
                    {(block.type === "image" || block.type === "video") && (
                      <div className="space-y-2">
                        <input
                          ...
                          className="w-full bg-black/50 border border-white/10 rounded px-2 py-1.5 text-small focus:border-purple-500 outline-none"
                        />
                        {block.type === "image" && (
                          <input
                    ...
                    {(block.type === "twoImages" ||
                      block.type === "compareFourImages" ||
                      block.type === "compareFiveImages" ||
                      block.type === "compareSevenImages") && (
                      <div className="grid grid-cols-2 gap-2">
                        {block.images.map((img, i) => (
                          <div
                            key={i}
                            className="bg-white/5 p-2 rounded border border-white/5 space-y-1"
                          >
                            ...
                            <input
                              ...
                              className="w-full bg-black border border-white/10 rounded px-2 py-1 text-small outline-none focus:border-purple-500"
                            />
                            <input
                              placeholder="Alt..."
                              value={img.alt}
                ...
                <div className="flex flex-wrap gap-2 pt-2">
                  <span className="text-small text-white/40 self-center mr-2">
                    Add Content:
                  </span>
                  {[
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/30 rounded text-small text-white transition-colors"
                    >
                      {btn.label}
                    </button>
                  ))}
      ...
      <div className="sticky bottom-0 bg-[#111] pt-4 border-t border-white/10 flex justify-between items-center z-10">
        <div className="text-small font-bold text-emerald-400">{message}</div>
        <button
          type="submit"
          disabled={saving}
